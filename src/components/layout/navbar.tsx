...
const navLinkHrefs = ["/", "/soluzioni", "/contact"] as const;
...
export function Navbar() {
  ...
  const navLinks = [
    ...
    { href: navLinkHrefs[1], label: t.nav.soluzioni },
    { href: navLinkHrefs[2], label: t.nav.contact },
  ];

  return (
    <header className="sticky top-0 z-50 w-full bg-ice/95 backdrop-blur-sm border-b border-border">
      <nav className="section-container">
        <div className="flex h-20 items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group" onClick={closeMenu}>
            ...
            <span className="text-xl font-sans font-semibold text-navy">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={cn(
                  "font-sans text-base transition-colors duration-300 hover:text-navy-light",
                  pathname === link.href
                    ? "text-navy-light font-medium"
                    : "text-navy"
                )}
              >
                {link.label}
